...
// --------------------Get program data------------------------
$('#program-form-data').submit(function(e){
    e.preventDefault(); 
     $.ajax({
         url: base_url + 'Programs/get_program_form_data/',
         type:"post",
         dataType: 'json',
...
// -----------------------After creating courses get subjects detail and forwad to function 
$('#course-related-subject-form').submit(function(e){
    e.preventDefault(); 
     $.ajax({
         url: base_url + 'Programs/get_program_related_subjects_data/',
         type:"post",
         dataType: 'json',
         data:new FormData(this),
         processData:false,
...
// ---------------recive one subject data to open on edit modal-----------
$(".get-edit-subject-data").on('click', function(){
    // e.preventDefault();
    var course_code = $(this).attr('data-course_code');
    var id = $(this).attr('data-id');
    var subject_code = $(this).attr('data-subject_code');
    var subject_grade = $(this).attr('data-subject_grade');
    var subject_semester = $(this).attr('data-subject_semester');
    var subject_type = $(this).attr('data-subject_type');
    var subject_name = $(this).attr('data-subject_name');
    var subject_type_no = (subject_type == `Main`) ? 0 : 1;
    if(subject_type == 'Main'){
        var other_subject_type = 'Elective';
        var other_subject_type_no = 1;
    }else{
        var other_subject_type = 'Main';
        var other_subject_type_no = 0;
    }

    // show the value on fee modal
    $('#edit-subject-data-modal').modal('show');
    
    $('[name="course_code"]').val(course_code);
    $('[name="id"]').val(id);
    $('[name="subject_code"]').val(subject_code);
    $('[name="subject_grade"]').val(subject_grade);
    $('[name="subject_semester"]').val(subject_semester);
    $('[name="subject_name"]').val(subject_name);
    $('select[name="subject_type"]').append('<option selected value="'+ subject_type_no +'">'+ subject_type +'</option><option value="'+ other_subject_type_no +'">'+ other_subject_type +'</option>');
//     $(this).closest('tr').after(`
//         <tr>
//             <form class="get-subject-data-after-edit" action="#" method="post">
//                 <td><button type="submit" class="btn btn btn-block btn-success">Submit</button></td>
//                 <td>`+no+`</td>
//                 <td><input type="text" value="`+subject_code+`" name="subject_code" class="form-control"></td>
//                 <td><input type="text" value="`+subject_grade+`" name="subject_grade" class="form-control"></td>
//                 <td><input type="text" value="`+subject_semester+`" name="subject_semester" class="form-control"></td>
//                 <td><select class="form-control select2" name="subject_type" style="width: 100%;">
//                             <option value="`+subject_type_no+`">`+subject_type+`</option>
//                             <option value="`+other_subject_type_no+`">`+other_subject_type+`</option>
//                         </select></td>
//                 <td><input type="text" value="`+subject_name+`" name="subject_name" class="form-control"></td>
//             </form>
//           </tr>
//     `).hide();
//     return false;
...
// $('.get-edit-subject-data').on('click', function (e) {
//     e.preventDefault();
//     $()
...
//---------END ON edit single subject model---------

// --------get data of single subject edit form data-----------------
$('.edit-subject-data-related-to-course').submit(function(e){
    e.preventDefault(); 
     $.ajax({
         url: base_url + 'Programs/get_edit_subject_data/',
         type:"post",
         dataType: 'json',
         data:new FormData(this),
         processData:false,
...
// --------delete subject data-----------------
$('.delete-subject-function').on('click',function(e){
    e.preventDefault(); 
    var id = $(this).attr('data-id');
    var course_code = $(this).attr('data-course_code');
     $.ajax({
        url: base_url + 'Programs/delete_subject/',
        type: 'post',
        data: {subject_id: id, course_code: course_code},
        dataType: 'json',
        success: function(data){
            // alert(data.success);
            console.log(data);
